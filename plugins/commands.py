...
# Ask Doubt on telegram @BlazeTitan
...
async def start(client, message):
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    ...
                    InlineKeyboardButton("𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥", url="t.me/BlazeTitan")
                ],[
                    InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @BlazeTitan
                ]]
            else:
                button = [[
                    InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}'),
    ...
    if not files_:
        ...
        try:
            ...
            title = '@FilmyvibeOn  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@FilmyvibeOn  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@FilmyvibeOn  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
    ...
    if SHORTLINK_MODE == True:
        buttons = [
            [
                InlineKeyboardButton(
                    'Rᴇsᴜʟᴛ Pᴀɢᴇ',
                    callback_data=f'setgs#Tᴇxᴛ#{settings["text"]}#{grp_id}',
                ),
                InlineKeyboardButton(
                    'Tᴇxᴛ' if settings["text"] else 'Bᴜᴛᴛᴏɴ',
                    callback_data=f'setgs#text#{settings["text"]}#{grp_id}',
                ),
            ],
            [
                InlineKeyboardButton(
